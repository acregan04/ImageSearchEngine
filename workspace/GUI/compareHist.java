...
public class compareHist {
    public ArrayList<Double>[] run(ArrayList<float[][][]> imageMatrices, float[][][] qHsv) {
    	...
    	for (int i=0; i<imageMatrices.size(); i++) {	
    		...
    		float[][][] srcTest = imageMatrices.get(i);
    		float[][][] histTest = histogram(srcTest, numBins, numBins, numBins); 
            System.out.println("\n");
    ...
    /*
    public static int[] histogram(int[][] matrix, int numBins) {
        ...
        double binSize = (max - min + 1) / (double) numBins;
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                int bin = (int) ((matrix[i][j] - min) / binSize);
                hist[bin]++;
            }
        }
        return hist;
